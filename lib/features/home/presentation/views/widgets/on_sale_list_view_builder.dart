...
//   const OnSaleListViewBuilder({super.key, required this.child});
//   final Widget child;
//   @override
...
//           //   );
//           // }
//           return Builder(builder: state.products!);
//           //  child({products: state.products});
//           // عرض الشبكة باستخدام SliverGrid
//           //     return OnSaleListView(products: state.products);
//         }
//         // أي حالة أخرى
//         else {
//           return const SliverToBoxAdapter(
...
//   const Child({super.key, required this.product, required this.child});
...
// class OnSaleListViewBuilder extends StatelessWidget {
//   const OnSaleListViewBuilder({super.key, required this.builder});
//   final Widget Function(List<ProductEntity> products) builder;
    ...
    return BlocConsumer<ProductsCubit, ProductsState>(
      ...
      builder: (context, state) {
        if (state is ProductsLoading) {
          ...
          // ✅ فلترة المنتجات يلي عليها عرض فقط
          final onSaleProducts = state.products
              .where((product) => product.isOnSale)
              .toList();

          return builder(onSaleProducts);
        } else {
          return const Center(
            child: Padding(
              padding: EdgeInsets.all(20.0),
